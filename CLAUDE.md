...
- **テーマは 2 系統ある（2026-08-18〜）。**
  - お客さん側 = 和モダン（生成り／きつね色／明朝）。`:root` のトークンが正
  - スタッフ側 = SnowUI トンマナ（白基調／黒の透過階調／角丸大きめ）。
    `body.theme-staff` のトークン上書きで実現しており、部品の CSS は共通。
    **スタッフ側の見た目を変えるときは `.theme-staff` ブロックの変数を触る**
