...
class CategoryGroupPickerTest {
    ...
    void theNewScreenOffersThePicker() throws Exception {
        ...
        // ★ 画面が最後まで描けていること（2026-09-20 に追加）。
        //
        //   もとは th:value="${T(...CategoryForm).NEW_GROUP}" と書いていました。
        //   動いているアプリのクラスにその定数がまだ無いと（＝Java を直して
        //   再起動する前）、式がそこで落ちて<b>画面が途中で切れます</b>。
        //   200 で返り始めたあとなので<b>エラー画面にもなりません</b>。
        //   店主には「新規が大カテゴリしかないよ」という形で見えました。
        //
        //   選択肢の有無だけ見ていると、切れていても気づけません。
        //   いちばん後ろまで届いていることを確かめます。
        assertThat(main).as("★ カテゴリ名の欄が無い。画面が途中で切れている")
                .contains("name=\"name\"");
        assertThat(main).as("★［追加する］が無い。画面が途中で切れている")
                .contains("追加する");
        assertThat(main).as("★ 末尾の注記まで届いていない。画面が途中で切れている")
                .contains("大分類は打ち間違えるとメニューのタブが割れる");
    }
