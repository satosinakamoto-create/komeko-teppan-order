...
/**
 * しぼり込みの 2 つの箱は同じ高さ（2026-09-14）。
 *
 * <p><b>★ 見ている画面が変わりました（2026-09-16）。</b>
 * もともとの根拠は ト07 食材・在庫（725:4468）でしたが、食材側の分類をやめたので
 * あちらの箱は「食材を探す」1 つだけになりました
 * （{@code IngredientCategoryRemovedTest}）。
 * <b>このテストは残します。</b>{@code .searchbox} と {@code .catpick} は
 * <b>品切れ・残数（現03）が商品カテゴリの絞り込みで使い続けている</b>ので、
 * 2 つの箱が横に並ぶ形はそちらに残っているためです。
 * 見ているのは CSS の共通クラスなので、対象画面が減っても効きます。
 *
 * <p><b>Figma の実測（当時・ト07 食材・在庫）</b>
 * <pre>
