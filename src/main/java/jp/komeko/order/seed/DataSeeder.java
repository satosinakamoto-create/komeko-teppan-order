 ...
 *       アレルギーの表示は何も出ません（お客さまには店頭で聞いていただく）。</li>
...
public class DataSeeder implements ApplicationRunner {
     ...
     *
     * <p><b>★ カウンターは「席」ではなく「札」です（2026-09-06 の判断）</b><br>
     * テーブル席は組と席が自然に一致するので、QR を卓に貼れば済みます。
     * <b>カウンターは一致しません。</b>1 人で 1 席のときも、3 人で 3 席のときもあり、
     * 組の単位が固定されていないためです。
     *
     * <p>席ごとに QR を貼ると、3 人組が座ったときに<b>伝票が 3 枚に割れます</b>。
     * 全員で 1 つの QR を読めば金額は正しく出ますが、
     * 隣の席に座った人は<b>目の前の QR を読みます</b>。そちらは別の卓なので
     * 注文が分かれ、しかも会計まで誰も気づきません。
     *
     * <p>そこでカウンターの QR は席に貼らず、<b>可動の札に印刷して組の前に置きます</b>。
     * 1 人でも 3 人でも札は 1 枚。自分の前に 1 枚しか無いので、
     * 隣の QR を読む事故が構造的に起きません。
     *
     * <p><b>枚数は席数と同じだけ要ります。</b>全席が 1 人客なら 6 組になるためです。
     * ただし出しておくのは使っている札だけなので、
     * カウンターに並ぶのは通常 2〜3 枚に収まります。
     *
     * <p>名前を「カウンター1」のままにしていないのは、
    ...
    private void seedTables() {
        ...
        int order = 0;
        // カウンターの札。定員は「1 枚の札で受けられる最大人数」なので、
        // 席数（1）ではなくカウンター全体の席数を入れておく。
        // ここを 1 にすると、3 人組に札を渡す運用と数字が食い違う
        for (int i = 1; i <= 6; i++) {
            order += 10;
            diningTableRepository.save(new DiningTable("カウンター札" + i, 6, order));
        }
        for (int i = 1; i <= 4; i++) {
            order += 10;
            diningTableRepository.save(new DiningTable("テーブル" + i, 4, order));
        }
        log.info("カウンターの札 6 枚とテーブル 4 卓を作成しました"
                + "（管理画面から店内配置に合わせて調整してください）");
    }
    ...
    private void seedServiceItems() {
        ...
        m.add("取り皿", 0, null);
        m.add("灰皿", 0, null);
        // 並びは設計（暗03）どおり。3 列に置いたとき
        // スタッフを呼ぶ／お水／おしぼり ・ 取り皿／灰皿／領収書 ・ 塩コショウ になる
        m.add("領収書", 0, null);
        m.add("塩コショウ", 0, null);
    }
    ...
    private String groupNameFor(int sortOrder) {
        if (sortOrder < 60) {
            // 広島風お好み焼き / 選べる米粉たこ焼き / 鉄板おつまみ / 鉄板麺 / 数量限定鉄板焼き。
            //
            // ★ 2026-09-20：最初の 2 つを「粉もの」から「鉄板料理」に寄せました（店主の判断）。
            //   どれも鉄板で焼く品なので、タブを分ける理由が弱く、
            //   分けるぶんお客さまの選択が 1 段増えていました。
            //
            //   本番の DB にはさらに「大分類＝お好み焼き／たこ焼き」が入っていました。
            //   カテゴリ名とほぼ同じで、大分類として意味を持っていません
            //   （昔の自由入力で入った値）。画面から直せるようにしたうえで寄せます。
            return "鉄板料理";
        }
        if (sortOrder < 80) {
            return "一品料理";      // 一品料理 / 甘味
