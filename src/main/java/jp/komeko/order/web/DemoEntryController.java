...
public class DemoEntryController {
    ...
    public String staffEntry(@RequestParam(name = "retry", required = false) String retry,
        ...
        // ── ログイン画面へ送る（2026-09-23・店主の指示）────────────────
        //
        // もとはここで説明カード（demo-staff.html）を出していました。
        // 入る前に「見学モード」「データは架空」を伝えるための 1 枚です。
        //
        // ログイン画面に「ゲストとして見る（店舗UI）」を置き、その下に同じ告知を
        // 書いたので、カードは役目を終えました。2 枚あると、
        // 見学者は同じ説明を 2 回読んでから入ることになります。
        //
        // ★ ここを転送にしたのは、ポートフォリオ側の HTML を触らずに済むからです。
        //   サイトのボタンは /demo/staff を指したままで、既存のリンクも QR も生きます。
        //   向こうは別の CLI で編集しており、こちらから確実に直せません。
        //
        // ★ ?retry=1 はそのまま持っていきます。
        //   CSRF が切れて戻ってきた人に「やり直せます」と伝える印で、
        //   落とすと 403 のあと何も案内が出ない画面に着きます
        //   （転送先の案内は login.html 側にあります）。
        return "redirect:/login" + (retry != null ? "?retry=1" : "");
    }
